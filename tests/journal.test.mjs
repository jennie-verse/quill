...
test('backup activity ledger accepts only Quill metadata records', () => {
  const record = mergeDocumentActivity(null, document, 'opened', '2026-08-17T09:00:00-05:00');
  assert.deepEqual(validateActivityLedger([record]), [record]);
  assert.throws(() => validateActivityLedger([{ ...record, kind: 'map-activity' }]), /Invalid Quill Journal activity ledger/);
});
...
test('new document sessions, explicit opens, edits, and exports are connected', async () => {
  ...
  assert.match(app, /journalActivity: Journal\.exportActivityLedger\(\)/);
  assert.match(app, /Journal\.replaceActivityLedger\(backup\.journalActivity\)/);
});

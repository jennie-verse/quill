...
import { validateActivityLedger } from './journal.js';
...
export function buildBackup({ draft, settings, journalActivity = [], snapshots = null }) {
  ...
  return JSON.stringify({
    ...
    journalActivity: validateActivityLedger(journalActivity),
    // Toggle in Settings → Data. Omitted entirely (not just empty) when off,
    // so turning it off actually shrinks the file.
    ...(normalizedSettings.includeSnapshotsInBackup && Array.isArray(snapshots)
...
export function parseBackup(rawText) {
  ...
  return {
    ...
    journalActivity: payload.journalActivity === undefined ? undefined : validateActivityLedger(payload.journalActivity),
    // undefined (key absent) means "this backup predates snapshots, or the
    // toggle was off" — callers must not wipe existing snapshots in that case.
    snapshots: Array.isArray(payload.snapshots) ? payload.snapshots.map(normalizeSnapshot).filter(Boolean) : undefined,
...
export function describeBackup(backup) {
  ...
  if (Array.isArray(backup.snapshots)) parts.push(`${backup.snapshots.length} snapshot${backup.snapshots.length === 1 ? '' : 's'}`);

...
export function mergeDocumentActivity(previous, document, action, at = new Date(), options = {}) {
  ...
  return { id: `${document.id}:${date}`, kind: 'file-activity', at: previous?.at && previous.at < timestamp ? previous.at : timestamp, updatedAt: timestamp, deleted: false, title: String(document.title || 'Untitled'), data: { itemId: String(document.id), itemType: 'text-document', actions: ORDER.filter(value => actions.has(value)), firstAt: previous?.data?.firstAt && previous.data.firstAt < timestamp ? previous.data.firstAt : timestamp, lastAt: previous?.data?.lastAt && previous.data.lastAt > timestamp ? previous.data.lastAt : timestamp, openCount: Math.max(0, Number(previous?.data?.openCount) || 0) + (action === 'opened' ? 1 : 0), ...(options.importedHistory ? { importedHistory: true } : {}) } };
}
